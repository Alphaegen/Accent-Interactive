...
if ($user==true) {
  if($user->EditUser($email, $fname, $infix, $lname, $telephone, $sname, $snumber, $postcode, $city, $country, $wrong_logins, $user_role, $confirmed, $id)) {
      die;
  } else {
      $user->printMsg();
      die;
  ...
  if($user->EditEvent($img, $eventname, $seats, $starttime, $endtime, $city, $address, $description, $smalldesc, $price, $id)) {
      die;
  } else {
      $user->printMsg();
      die;
  ...
  if($user->EditOrder($email, $eventname, $seats, $total, $id)) {
      die;
  } else {
      $user->printMsg();
      die;
